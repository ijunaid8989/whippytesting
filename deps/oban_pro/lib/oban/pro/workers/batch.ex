defmodule Oban.Pro.Workers.Batch do
  @moduledoc ~S"""
  A `Batch` worker links the execution of many jobs as a group and runs optional callbacks after
  jobs are processed. This allows your application to coordinate the execution of tens, hundreds
  or thousands of jobs in parallel.

  ## Installation

  Before running a `Batch` in production, you should run a migration to add an optimized index
  for batch queries. Without the optimization, batch queries may be slow:
  ...
  Note that we **define a `process/1` callback instead of `perform/1`** because `perform/1` is
  used behind the scenes to coordinate regular execution and callbacks within the same worker. The
  `process/1` function receives an `Oban.Job` struct, just like `perform/1` would and it should
  return the same accepted values, i.e. `:ok`, `{:ok, value}`, `{:error, error}`.
  ...
  Now the hypothetical `track_delivery/2` function will store the delivery details for retrieval
  later, possibly by one of our [handler callbacks](#handler-callbacks).

  ## Inserting Batches

  Create batches with `c:new_batch/1,2` by passing a list of args and options, or a list of
  heterogeneous jobs. A list of args and options is transformed into a list of jobs for that batch
  module. For example, this will build and insert two `EmailBatch` jobs:
  ...
  The `c:new_batch/1,2` callback automatically injects a unique `batch_id` into each job's meta. A
  `Batch` worker is a regular `Oban.Worker` under the hood, which means you can use `new/2` to
  insert jobs as well, provided you use a [deterministic batch id](#generating-batch-ids).
  ...
  By default a `batch_id` is generated as a time-ordered random [UUIDv7][uuid]. UUIDs are more
  than sufficient to ensure that batches are unique between workers and nodes for any period.
  However, if you require control, you can override `batch_id` generation at the worker level or
  pass a value directly to the `new_batch/2` function.

  To override the `batch_id` for a particular worker you override the `c:gen_id/0` callback:
  ...
  The `c:gen_id/0` callback is suited for random/non-deterministic id generation. If you'd prefer
  to use a deterministic id instead, you can pass the `batch_id` in as an option to `new_batch/2`:

  ```elixir
  MyApp.BatchWorker.new_batch(list_of_args, batch_id: "custom-batch-id")
  ```

  Using this technique you can verify the `batch_id` in tests or append to the batch manually
  after it was originally created. For example, you can add to a batch that is scheduled for the
  future:
