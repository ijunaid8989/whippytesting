defmodule Sync.Workers.Aqore.CustomData.JobCandidateTest do
  ...
  alias Sync.Workers.Aqore.CustomData.JobCandidates
  ...
  describe "process/1 for process_job_candidates_as_custom_object_records" do
    test "logs an error when the job candidate custom object is not yet synced to whippy" do
      ...
      assert capture_log(fn ->
               assert :ok ==
                        perform_job(JobCandidates, %{
                          "type" => "process_job_candidates_as_custom_object_records",
                          "integration_id" => integration.id
                        })
             end) =~ "job_candidate custom_object not synced to whippy."
    end

    test "logs an error when the job_candidate custom object is not found" do
      integration = insert(:integration)
      ...
      assert capture_log(fn ->
             ...
             end) =~ "No custom object found for job candidates"
    end

    test "finds the job candidate custom object and calls the converter with it", %{
      integration: integration,
      custom_object: custom_object
    } do
      insert(:contact,
    ...
    test "finds the latest job candidate custom object and calls the converter with it", %{
      integration: integration,
      custom_object: custom_object
    } do
      insert(:contact,
      ...
      with_mocks([
        ...
        assert :ok ==
                 perform_job(JobCandidates, %{
                   "type" => "process_daily_job_candidates_as_custom_object_records",
                   "integration_id" => integration.id
                 })
    ...
    test "logs an error when the latest job_candidate custom object is not found" do
      integration = insert(:integration)
      ...
      assert capture_log(fn ->
             ...
             end) =~ "Daily sync. No custom object found for job candidates"
    end

    test "logs an error when the latest job candidate custom object is not yet synced to whippy" do
      integration = insert(:integration)
    ...
    test "pulls job candidates from Aqore second time in daily sync when the response is an 15mins interval message", %{
      integration: integration
    } do
      with_mocks([
  ...
  defp http_poison_mocks do
    [
      post: fn _url, _headers, _opts, _ -> list_job_candidates_fixture() end
    ]
  end
