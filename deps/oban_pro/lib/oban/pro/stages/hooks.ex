defmodule Oban.Pro.Stages.Hooks do
  ...
  def init(worker, modules) do
    with :ok <- Utils.validate_opts(modules, &validate_opt/1) do
      modules =
        cond do
          function_exported?(worker, :after_process, 2) ->
            [worker | modules]

          function_exported?(worker, :after_process, 3) ->
            [worker | modules]
  ...
  def handle_event(_event, _timing, meta, _conf) do
    %{job: job, state: state, worker: worker} = meta
    ...
    with {:ok, worker} <- Oban.Worker.from_string(worker),
      ...
      module_hooks = get_in(opts, [:stages, __MODULE__, Access.key!(:modules)])
  ...
  defp validate_opt(modules) do
    ...
    |> Utils.validate_opts(fn module ->
      cond do
        ...
        true ->
          {:error, "#{inspect(module)} doesn't implement after_process/2,3"}
      end
    end)
  end
end
