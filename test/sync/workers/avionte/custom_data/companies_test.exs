defmodule Sync.Workers.Avionte.CustomData.CompaniesTest do
  ...
  alias Sync.Workers.Avionte.CustomData.AvionteCompanies
  ...
  describe "process/1 for process_companies_as_custom_object_records" do
    test "finds the companies custom object, pulls companies and calls the converter for each of them",
         %{
           integration: integration
         } do
      with_mocks([
        ...
        {HTTPoison, [],
         [
           get: fn url, _headers, _opts ->
             cond do
               url =~ "companies/ids" -> list_companies_ids_fixture()
             end
           end
         ]},
        {
          ...
          [
            post: fn url, _headers, _opts, _body ->
              cond do
                url =~ "/companies/multi-query" -> list_companies_fixture()
              end
            end
          ]
        }
        ...
        assert :ok ==
                 perform_job(AvionteCompanies, %{
                   "type" => "process_companies_as_custom_object_records",
    ...
    test "logs an error when the talent custom object is not found" do
      ...
      assert capture_log(fn ->
             ...
             end) =~ "No custom object found for companies."
    end
    ...
    test "logs an error when the talent custom object is not yet synced to whippy" do
      ...
      assert capture_log(fn ->
             ...
             end) =~ "companies custom_object not synced to whippy."
    end
  end
end
