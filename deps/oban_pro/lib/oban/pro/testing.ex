defmodule Oban.Pro.Testing do
  ...
  * `:with_summary` — whether to summarize execution results with a map of counts by state, or
  ...
  def run_workflow(%Workflow{} = workflow, opts \\ []) when is_list(opts) do
    ...
    [job | _rest] = Smart.insert_all_jobs(conf, workflow.changesets, [])

    opts
    |> Keyword.put_new(:queue, :all)
    |> Keyword.put(:with_summary, true)
    |> drain_jobs()

    # The workflow facilitator won't run "held" jobs, so we have to reload everything that was
    # inserted to get all jobs and their real fields.
    jobs = Workflow.all_jobs(%Job{job | conf: conf})
  ...
  defp assert_valid_callback(worker, callback) do
    ...
    function = Map.fetch!(Batcher.callbacks_to_functions(), to_string(callback))

    assert function_exported?(worker, function, 1), """
    Expected #{inspect(function)} callback to be implemented
    """
  end
  ...
  defp fetch_available(conf, opts) do
    init_opts = [queue: opts.queue, limit: opts.with_limit]

    {:ok, meta} = conf.engine.init(conf, init_opts)
