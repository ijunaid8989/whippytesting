defmodule Oban.Pro.Worker do
  ...
  * Uniqueness works for encrypted jobs, but not for arguments because the same args are encrypted
  ...
  _during_ its next run if desired.
  ...
  Deadlines may be set at runtime as well, provided the worker was _already_ configured with a
  `deadline` option.
  ...
  deadline to consider scheduling—a job scheduled to run 1 hour from now with a 1 hour deadline
  will expire 2 hours in the future. Note that deadlines only account for scheduling on insert,
  not on retry or after a snooze.

  ### Applying Deadlines to Running Jobs

  A job that has already started may be cancelled at runtime if it won't complete before the
  deadline. Consider a slower job that takes 5 minutes to execute and has a deadline 10 minutes in
  the future. If the job starts 9 minutes from now it wouldn't finish until 4 minutes past the
  deadline.
  ...
  There are three mechanisms for defining and attaching an `c:after_process/3` hook:
  ...
  * `snooze`—when a job returns `{:snooze, seconds}`
  ...
  alias Oban.Pro.Stages.{Deadline, Encrypted, Hooks, Recorded, Standard, Structured}
  ...
  @optional_callbacks after_process: 2, after_process: 3, fetch_recorded: 1

  @doc false
  defmacro __using__(opts) do
    stand_opts = Keyword.drop(opts, [:deadline, :encrypted, :hooks, :recorded])
    ...
    stage_opts = [
      ...
      {Structured, []},
      {Deadline, Keyword.get(opts, :deadline, :ignore)},
