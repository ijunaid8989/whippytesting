defmodule Sync.Workers.Avionte.AvionteContactsTest do
  ...
  describe "process/1" do
    test "pulls contacts from Avionte and saves them as contacts", %{integration: integration} do
      with_mock(HTTPoison, [], httpoison_mock(:avionte_list, :success)) do
        ...
        assert :ok ==
                 perform_job(AvionteContacts, %{
                   "type" => "pull_contacts_from_avionte",
    ...
    test "pulls contacts from Avionte and saves them as contacts with external_contact_hash and sync_to_whippy", %{
      integration: integration
    } do
      with_mock(HTTPoison, [], httpoison_mock(:avionte_list, :success)) do
        insert(:contact,
    ...
    test "pulls contacts from Avionte and skips a batch of Contact IDs after retry attempts are exhausted", %{
      integration: integration
    } do
      with_mock(HTTPoison, [], httpoison_mock(:avionte_list, :failure)) do
        ...
        assert captured_log =~ "Skipping batch of contacts:"
      end
    end

    test "pulls contacts from Avionte and skips archived contacts", %{integration: integration} do
      with_mock(HTTPoison, [], httpoison_mock(:avionte_list, :success)) do
        assert [] == Repo.all(Contact)
    ...
    test "pushes Avionte contacts into Whippy", %{integration: integration} do
      with_mock(HTTPoison, [],
    ...
    test "pushes Avionte contacts into Whippy and set should_sync_to_whippy to false", %{integration: integration} do
      with_mock(HTTPoison, [],
        request: fn :post, _url, body, _header, _opts ->
          expected_body = %{
            "address" => %{
    ...
    test "pushes Avionte contacts into Whippy and set should_sync_to_whippy to false for branched integration", %{
  ...
  defp httpoison_mock(:avionte_list, type) do
    list_contacts_fixture =
      case type do
        :success ->
          Fixtures.AvionteClient.list_contacts_fixture()
    ...
    [
      post: fn _url, _params, _headers, _opts -> list_contacts_fixture end,
