defmodule Oban.Pro.Workers.Chain do
  ...
  * `:wait_snooze` — the number of seconds a job will snooze after awaiting `executing` upstream
  ...
  It's inefficient to have jobs in the same chain run concurrently because only the head of the
  chain will actually execute. To optimize throughput while allowing concurrent execution, you can
  partition the queue using the same options as the chain.

  For example, here's how to configure a queue for a chain that's partitioned by `worker` and
  `:user_id`, to allow 20 concurrent jobs per-node but only 1 globally for that partition:
