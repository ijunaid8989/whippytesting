defmodule Sync.Workers.Tempworks.CustomData.CustomersTest do
  ...
  alias Sync.Workers.Tempworks.CustomData.Customers
  ...
  describe "process/1 for process_customers_as_custom_object_records" do
    setup %{
      integration: integration,
      custom_object: custom_object
    } do
      ...
      _foreign_assoc_property =
        insert(:custom_property, custom_object: custom_object, whippy_custom_property: %{"label" => "customerId"})

      :ok
    end

    test "finds the customer_custom_data custom object, pulls customer custom data and calls the converter for each of them",
         %{
           integration: integration
         } do
      with_mocks([
        ...
        {HTTPoison, [],
         [
           get: fn url, _headers, _opts ->
             cond do
               url =~ "Search" -> list_customer_fixture()
               url =~ "CustomData" -> customer_custom_data_fixture()
             end
           end
         ]}
      ]) do
        assert :ok ==
                 perform_job(Customers, %{
                   "type" => "process_customers_as_custom_object_records",
