defmodule Oban.Pro.Plugins.DynamicPruner do
  ...
  Pruning checks the `scheduled_at` timestamp for optimized, index-backed queries by default. In
  rare situations where the `scheduled_at` timestamp isn't accurate enough to identify prunable
  jobs, e.g. cancelling large swaths of jobs scheduled far into the future, you can use the
  `by_state_timestamp` option for increased accuracy.
  ...
  * `:pruned_jobs` - the jobs that were deleted from the database
  ...
  defp delete_for_mode(query, {:max_age, age}, state) do
    ...
    query = select(query, [j], %{id: j.id, queue: j.queue, state: j.state, rn: 100_000_000})

    if state.by_state_timestamp do
      query
