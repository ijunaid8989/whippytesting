defmodule Sync.Workers.Aqore.CustomData.CandidatesTest do
  ...
  alias Sync.Workers.Aqore.CustomData.Candidates
  ...
  describe "process/1 for process_candidates_as_custom_object_records" do
    test "finds the candidate custom object and calls the converter with it", %{
      integration: integration,
      custom_object: custom_object
    } do
      with_mock(Converter, [],
        ...
        assert :ok ==
                 perform_job(Candidates, %{
                   "type" => "process_candidates_as_custom_object_records",
    ...
    test "logs an error when the candidate custom object is not found" do
      ...
      assert capture_log(fn ->
             ...
             end) =~ "No custom object found for candidate."
    end

    test "logs an error when the candidate custom object is not yet synced to whippy" do
      integration = insert(:integration)
      ...
      assert capture_log(fn ->
             ...
             end) =~ "candidate custom_object not synced to whippy."
    end
  end
end
