defmodule Sync.Workers.Avionte.CustomData.Talents do
  @moduledoc """
  ...
  def process(
        %Job{args: %{"type" => "process_talents_as_custom_object_records", "integration_id" => integration_id}} = _job
      ) do
    Logger.info("[Avionte] [Integration #{integration_id}] Converting talents to custom object records.")
    ...
    Enum.each(integrations, fn integration ->
      case Contacts.list_custom_objects_by_external_entity_type(integration, "talent") do
        [] ->
          Logger.error("[Avionte] [Integration #{integration_id}] No custom object found for talent.")
        ...
        [_custom_object] ->
          Logger.error("[Avionte] [Integration #{integration_id}] talent custom_object not synced to whippy.")

        _ ->
          Logger.error("[Avionte] [Integration #{integration_id}] Multiple custom objects found for talent.")
      end
    end)
  ...
  defp build_condition(integration) do
    ...
    client_condition = dynamic([c], not like(c.external_contact_id, "contact-%"))
